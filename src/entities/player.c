...
void handleInput(const char input, Player *player) {
    switch (input) {
        case 'W':
        case 'w':
            moveEntity(player->attributes.position.y - 1, player->attributes.position.x, &player->attributes.position,
                       player->attributes.symbol);
            break;

        case 'A':
        case 'a':
            moveEntity(player->attributes.position.y, player->attributes.position.x - 1, &player->attributes.position,
                       player->attributes.symbol);
            break;
